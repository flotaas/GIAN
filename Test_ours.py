...
# 固定 augment_rate=0.1 for mosi
# 固定 augment_rate=0.5 for mosei

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('-m', '--modelName', type=str, default='HGCN', help='Name of model')
    parser.add_argument('-d', '--datasetName', type=str, default='mosi', choices=['mosi', 'mosei'],
                        help='Name of dataset')
    parser.add_argument('--augmentation', type=str, default='method_one',
                        help='support method_one/method_two/method_three')
    parser.add_argument('--augment_rate', type=float, default=0.1, help='0.1, 0.5')
    parser.add_argument('-g', '--gpu-ids', action='append', default=[])
    parser.add_argument('--res_save_dir', type=str, default='./', help='path to save results.')
    parser.add_argument('--noise_type', type=str, default='temporal_feature_missing',
                        help='support temporal_feature_missing/static_block_drop/static_random_drop')
    parser.add_argument('--model_save_path', type=str, default='./results\saved_models',
                        help='dirpath to the pretrained save results.')
    parser.add_argument('--noise_seed_list', type=list, default=[1, 11, 111, 1111, 11111],
                        help='indicates the seed for test period imperfect construction')
    return parser.parse_args()


def reproduce(args):
    model_save_path = os.path.join(args.model_save_path, 'normals',
                                   f'{args.modelName}-{args.datasetName}-{args.augmentation}-{args.augment_rate}-{args.seed}.pth')
...
def run_normal(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        if args.noise_type in ['temporal_feature_missing', 'static_block_drop', 'static_random_drop']:
            result_cur = dict()
        ...
        criterions = list(model_results[0].keys()) if args.noise_type != 'static_entire_drop' else list(
            model_results[0]['T_D'].keys())

...
class MMDataset(Dataset):
    def __init__(self, args, mode='train', **kwarg):
        ...
        self.augment_rate = args.augment_rate
        self.augment_seed = 2222
    ...
    def __init_mosi(self):
        ...
        if self.mode == 'train' or self.mode == 'valid':
            #print(self.args.augment) 'method_one'
            if self.args.augment == 'none':
                ...
                self.augment_low_text, self.augment_low_vision, self.augment_low_audio, self.text_missing_mask, self.audio_missing_mask, \
                    ...
                    self.augment(self.augment_rate, self.augment_seed, augment_rate=1.0, missing_strategy='structure')

            elif self.args.augment == 'method_two':
                self.augment_low_text, self.augment_low_vision, self.augment_low_audio, self.text_missing_mask, self.audio_missing_mask, \
                    self.vision_missing_mask, self.text_mask, self.audio_mask, self.vision_mask = \
                    self.augment(self.augment_rate, self.augment_seed, augment_rate=1.0, missing_strategy='random')

            elif self.args.augment == 'method_three':
                self.augment_low_text, self.augment_low_vision, self.augment_low_audio, self.text_missing_mask, self.audio_missing_mask, \
                    self.vision_missing_mask, self.text_mask, self.audio_mask, self.vision_mask = \
                    self.augment(self.augment_rate, self.augment_seed, augment_rate=1.0, missing_strategy='block')
            else:

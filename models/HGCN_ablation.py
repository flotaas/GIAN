...
class HyperGCN(nn.Module):
    def __init__(self, args):
        ...
        self.hypergraph_v = HypergraphConv(in_features=self.args.fus_d_v, out_features=args.fus_d_v, edges=64,
                                           filters=128)
        self.hypergraph_l = HypergraphConv(in_features=self.args.fus_d_l, out_features=args.fus_d_l, edges=64,
                                           filters=128)
...
class MGAT_v2(nn.Module):
    def __init__(self, args):
        super(MGAT_v2, self).__init__()
        self.args = args
        # BERT SUBNET FOR TEXT
        self.text_model = BertTextEncoder(language=args.language, use_finetune=args.use_bert_finetune)
        args.fusion_dim = args.fus_d_l + args.fus_d_a + args.fus_d_v
...
class LSTM_fusion(nn.Module):
    def __init__(self, args):
        ...
        self.fusion = nn.LSTM(input_size=args.fus_d_l + args.fus_d_a + args.fus_d_v,
                              hidden_size=args.fus_d_l + args.fus_d_a + args.fus_d_v, num_layers=args.fus_layers)

    def forward(self, text_x, audio_x, video_x):
        # (B, D, T) -->  (T, B, D)
        text_x, audio_x, video_x = text_x.permute(2, 0, 1), audio_x.permute(2, 0, 1), video_x.permute(2, 0, 1)
        trans_seq = self.fusion(torch.cat((text_x, audio_x, video_x), dim=2))
        if type(trans_seq) == tuple:
            trans_seq = trans_seq[0]
        return trans_seq[0]  # Utilize the [CLS] of text for full sequences representation.
...
class HGCN_wo_TGCN(nn.Module):

    def __init__(self, args):
        super(HGCN_wo_TGCN, self).__init__()

        self.args = args
        self.MGCN = MGAT_v2(args)
...
class HGCN_wo_MGCN(nn.Module):

    def __init__(self, args):
        super(HGCN_wo_MGCN, self).__init__()
...
class HGCN_wo_fusion(nn.Module):

    def __init__(self, args):
        super(HGCN_wo_fusion, self).__init__()

        self.args = args
        self.TGCN = HyperGCN(args)
        self.MGCN = MGAT(args)
        self.fusion = LSTM_fusion(args)
        self.classifier = classifier(args)

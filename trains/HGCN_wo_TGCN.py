...
from utils.functions import dict_to_str, mmd_loss, kl_divergence, js_divergence
...
class HGCN_wo_TGCN():
    ...
    def do_train(self, model, dataloader):
        if self.args.use_bert_finetune:
            ...
            bert_params = list(model.Model.MGCN.text_model.named_parameters())
        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    proj_l_all_lm, proj_a_all_lm, proj_v_all_lm = model.Model.MGCN(text_lm, audio_lm, vision_lm)
                    ...
                    # MGCN loss
                    mrl = self.pixelwise_loss(proj_l_all.detach(), proj_l_all_lm) + self.pixelwise_loss(
                        proj_a_all.detach(), proj_a_all_lm) \
                    ...
                    tot_loss = self.args.weight * (self.args.alpha * frl + (1 - self.args.alpha) * mrl) + c_loss
                    tot_loss.backward()
            ...
            logger.info("TRAIN-(%s) (%d/%d/%d)>> closs: %.4f mrloss: %.4f frloss: %.4f %s"
                        % (self.args.modelName, epochs - best_epoch, epochs, self.args.cur_time,
                           np.mean(avg_closs), np.mean(avg_mrloss), np.mean(avg_frloss), dict_to_str(train_results)))
            # validation

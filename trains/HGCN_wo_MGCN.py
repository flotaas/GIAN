        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    fusion_feature_lm = model.Model.fusion(proj_l_lm, proj_a_lm, proj_v_lm)
                    ...
                    # TGCN loss
                    trl = self.pixelwise_loss(proj_l.detach(), proj_l_lm) + self.pixelwise_loss(proj_a.detach(),
                                                                                                proj_a_lm) \
                    ...
                    tot_loss = self.args.weight * (self.args.alpha * frl + (1 - self.args.alpha) * trl) + c_loss
                    tot_loss.backward()
            ...
            logger.info("TRAIN-(%s) (%d/%d/%d)>> closs: %.4f trloss: %.4f frloss: %.4f %s"
                        % (self.args.modelName, epochs - best_epoch, epochs, self.args.cur_time,
                           np.mean(avg_closs), np.mean(avg_trloss), np.mean(avg_frloss), dict_to_str(train_results)))
            # validation
